...
#include "@{TPL_MODU_NAME}InteractiveTool.h"
...
#define LOCTEXT_NAMESPACE "U@{TPL_MODU_NAME}InteractiveTool"

/*
 * ToolBuilder
 */

UInteractiveTool* U@{TPL_MODU_NAME}InteractiveToolBuilder::BuildTool(const FToolBuilderState & SceneState) const
{
	U@{TPL_MODU_NAME}InteractiveTool* NewTool = NewObject<U@{TPL_MODU_NAME}InteractiveTool>(SceneState.ToolManager);
...
U@{TPL_MODU_NAME}InteractiveToolProperties::U@{TPL_MODU_NAME}InteractiveToolProperties()
{
...
void U@{TPL_MODU_NAME}InteractiveTool::SetWorld(UWorld* World)
{
	check(World);
...
void U@{TPL_MODU_NAME}InteractiveTool::Setup()
{
	...
	Properties = NewObject<U@{TPL_MODU_NAME}InteractiveToolProperties>(this, "Measurement");
...
void U@{TPL_MODU_NAME}InteractiveTool::OnUpdateModifierState(int ModifierID, bool bIsOn)
{
...
FInputRayHit U@{TPL_MODU_NAME}InteractiveTool::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
...
void U@{TPL_MODU_NAME}InteractiveTool::OnClickPress(const FInputDeviceRay& PressPos)
{
...
void U@{TPL_MODU_NAME}InteractiveTool::OnClickDrag(const FInputDeviceRay& DragPos)
{
	UpdatePosition(DragPos.WorldRay);
}


FInputRayHit U@{TPL_MODU_NAME}InteractiveTool::FindRayHit(const FRay& WorldRay, FVector& HitPos)
{
...
void U@{TPL_MODU_NAME}InteractiveTool::UpdatePosition(const FRay& WorldRay)
{
...
void U@{TPL_MODU_NAME}InteractiveTool::UpdateDistance()
{
	Properties->Distance = FVector::Distance(Properties->StartPoint, Properties->EndPoint);
}


void U@{TPL_MODU_NAME}InteractiveTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
...
void U@{TPL_MODU_NAME}InteractiveTool::Render(IToolsContextRenderAPI* RenderAPI)
{
